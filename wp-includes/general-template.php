...
function wp_login_form( $args = array() ) {
	$defaults = array(
		...
		'form_id'        => 'loginform',
		'label_username' => __( 'Username o Indirizzo Email' ),
		'label_password' => __( 'Password' ),
		'label_remember' => __( 'Ricorda' ),
...
function wp_register( $before = '<li>', $after = '</li>', $display = true ) {
	if ( ! is_user_logged_in() ) {
		if ( get_option( 'users_can_register' ) ) {
			$link = $before . '<a href="' . esc_url( wp_registration_url() ) . '">' . __( 'Non hai un account? Registrati' ) . '</a>' . $after;
		} else {
			$link = '';
		}
